...
def pybullet_mat_to_numpy_4x4(pybullet_matrix):
  ...
  return np.transpose(
      np.reshape(np.array(pybullet_matrix, dtype=np.float64), (4, 4)))
...
def image_xy_to_view_ray(xy, cam_width, cam_height, proj_mat_inv):
  ...
  xyz_ndc = np.array([2.0 * xy[0] / cam_width - 1.0,
                      -(2.0 * xy[1] / cam_height - 1.0),
                      0])  # in [-1, 1]
...
def ray_to_plane_test(ray_origin,
                      ray_vec,
                      plane_origin,
                      plane_normal):
...
def get_workspace(env):
  (workspace_origin,
   workspace_quat) = env.pybullet_client.getBasePositionAndOrientation(
       env.workspace_uid)
  workspace_normal = rotation_to_matrix(transform.Rotation.from_quat(
      workspace_quat))[2, 0:3]
...
def reset_camera_pose(env, view_type):
  ...
  if view_type == 'POLICY':
    ...
    euler = matrix_to_rotation(
        pybullet_mat_to_numpy_4x4(viewm)[0:3, 0:3]).as_euler(
            'xyz', degrees=False)
    pitch = euler[1]
    yaw = -euler[2]
...
class ObjState():
  ...
  def _get_joint_info(client, obj_id, joint_index):
    ...
    return tuple(
        [v if not isinstance(v, bytes) else v.decode('utf-8') for v in ji])
  ...
  def set_bullet_state(self, client, obj_id):
    ...
    for i, (joint_info, joint_state) in enumerate(
      ...
      if cur_joint_info != joint_info:
        raise ValueError('joint_info mismatch %s vs %s (expected)' % (
  ...
  def deserialize(data):
    return ObjState(obj_id=_lists_to_tuple(data['obj_id']),
...
class XarmState(ObjState):
  ...
  @staticmethod
  def deserialize(data):
    goal_translation = (None if not data['goal_translation'] else
                        _lists_to_tuple(data['goal_translation']))
    return XarmState(
        obj_id=data['obj_id'],
        base_pose=_lists_to_tuple(data['base_pose']),
        base_vel=_lists_to_tuple(data['base_vel']),
...
def _serialize_pybullet_state(pybullet_state):
  ...
  if isinstance(pybullet_state, list):
    ...
    return {key: _serialize_pybullet_state(value)
    ...
    raise ValueError('Unhandled type for object %s, type %s' %
...
def _deserialize_pybullet_state(state):
  ...
  if isinstance(state, list):
    ...
    if '_serialized_obj_name' in state:
      ...
      return {key: _deserialize_pybullet_state(value)
...
def write_pybullet_state(filename, pybullet_state, task,
                         actions = None):
...
def read_pybullet_state(filename):
  ...
  if data['state_version'] != PYBULLET_STATE_VERSION:
    raise ValueError('incompatible state data (version %d, expected %d)' % (
