...
class MultimodalOrientedPushOracle(
    oriented_push_oracle_module.OrientedPushOracle):
  """Oracle for multimodal pushing task."""

  def __init__(self, env, goal_dist_tolerance=0.04,
  ...
  def _action(self, time_step,
    ...
    if time_step.is_first():
      ...
      ((self._first_block, self._first_target),
       (self._second_block, self._second_target)) = self._choose_goal_order()
    ...
    if (_block_target_dist(self._first_block, self._first_target)
        < self._goal_dist_tolerance and not self._has_switched):
