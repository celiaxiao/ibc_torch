...
def train(exp_name, dataset_dir, image_obs, task, goal_tolerance):
    ...
    network = mlp_ebm.MLPEBM((act_shape[0]+10), 1, width=width, depth=depth,
    ...
    for epoch in tqdm.trange(num_epoch):
        ...
        if epoch % 100 == 0:
            policy_eval.evaluate(eval_episodes, 'PUSH', False, False, False, 
                static_policy=ibc_policy, video=False, output_path=path+str(epoch))
        if epoch % 500 == 0 and epoch != 0:
            torch.save(network.state_dict(), path+str(epoch)+'.pt')

def train_mse(exp_name, dataset_dir, image_obs, task, goal_tolerance):
...
if __name__ == '__main__':
    ...
    exp_name='multiple_push'
    dataset_dir = 'data/block_push_states_location/multiple_push.pt'
    image_obs = False
    task = "PUSH"
    goal_tolerance = 0.02
    # train(exp_name, dataset_dir, image_obs, task, goal_tolerance)
    train_mse(exp_name, dataset_dir, image_obs, task, goal_tolerance)
    # eval_mse(exp_name)