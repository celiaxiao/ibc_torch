...
from tests.test_mcmc import _get_mock_energy_network, _get_mock_energy_network_tf
...
def test_emb_loss():
    ...
    # print("loss difference", loss_torch.numpy(),'\n', loss_tf.numpy(), np.max(loss_torch.numpy() - loss_tf.numpy()))
    # np.allclose(loss_torch.numpy() - loss_tf.numpy())
    assert np.allclose(loss_torch.numpy(), loss_tf.numpy(), 1e-4) 
...
def test_gradient_loss():
    ...
    assert np.allclose(grad_torch.numpy(), grad_tf.numpy()) 
    ...
    assert np.allclose(grad_torch_1.numpy(), grad_tf_1.numpy()) 
    ...
    assert np.allclose(grad_torch_inf.numpy(), grad_tf_inf.numpy()) 
...
def test_gradient_loss_chain():
    ...
    _, chain_data_torch = langevin_actions_given_obs(
        ...
        policy_state=(),
        min_actions=torch.tensor([0., 0.]),
        max_actions=torch.tensor([1., 1.]),
        training=False,
        num_iterations=25,
